...
[![Tests](https://img.shields.io/badge/tests-60%20passing-success)](docs/test-strategy.md)

GitHub Search API を用いた **リポジトリ検索 Web アプリ**。
Next.js 16 (App Router) + TypeScript + Tailwind v4 で実装しています。

> 動作だけでなく、設計判断・テスト戦略・コミット粒度・ドキュメントの一貫性を重視して進めました。
> 設計の根拠は本 README と [`docs/test-strategy.md`](docs/test-strategy.md) に記載しています。
...
Conventional Commits + 日本語で、1 コミット = 動く / レビュー可能な単位として段階的に積み上げています。
特に「`watchers_count` → `subscribers_count` の修正」は **独立した `fix:` コミット** として残しており、調査の経緯と修正点をコミット履歴から直接辿れるようにしています。
...
本プロジェクトでは AI をコーディング補助として活用しつつ、設計判断・テスト戦略・実装方針は自身で主導しました。
透明性のため、活用範囲を以下の 3 区分で整理しています。

### AI に補助を依頼した範囲

- `create-next-app` のスキャフォールド後のボイラープレート整理
- ユーティリティ関数の初期実装案 (`format-number.ts` / `buildPageItems` 等)
- Tailwind クラスの整列 (`prettier-plugin-tailwindcss` 経由)
- 各種ドキュメントの文体整え・章立て
- テストケースの境界値網羅の洗い出し補助

### 自分で判断・主導した範囲

- 設計方針の選定 (feature-based 構成、Server Component first、URL = State、状態管理ライブラリ不採用)
- 認証方式の選定 (Basic Auth / NextAuth / セッション cookie の比較検討と Basic Auth 採用)
- ページネーション vs 無限スクロールの比較と選定根拠
- **テスト戦略**: 何をテストし何をテストしないか、重点領域 3 つの選定とドキュメント化
- **`watchers_count` の罠** の発見・調査・対応設計 (独立 `fix:` コミットを切る判断含む)
- コミット粒度・順序の設計 (Phase 0〜5 への分割)
- v16 の breaking changes (`proxy.ts` / async `searchParams` 等) への対応方針
...
- 例外型階層の粒度
- Server Component のテスト方針 (公式推奨を踏まえた「純関数化して単体テスト」)
- Vitest + Vite + Node 24 の互換問題切り分けと回避策

実装の進め方および試行錯誤の経緯は、コミット履歴 (`git log`) から確認できます。
